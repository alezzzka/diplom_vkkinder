...
while True:
    ...
    if message == "начать":
        ...
        if dict_blacklist == {}:
            write_msg(user_id, "У вас нет анкет в черном списке. Добавить анкету?")
        else:
            if black_list not in dict_blacklist.keys():
                write_msg(user_id, f"Анкеты в черном списке закончились. Для просмотра сначала введите 'с начала черного списка'")
            else:
        ...
        black_list = 1
        write_msg(user_id, "Посмотреть черный список?")
